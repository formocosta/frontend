...
import {
  BookOpen, Plus, Edit, Trash2, ChevronDown,
  ChevronRight, FolderOpen, LayoutGrid
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
              ...
              <div key={cat.id} className="bg-white rounded-md border border-gray-100 shadow-[0_4px_24px_rgba(0,0,0,0.02)] overflow-hidden transition-all hover:border-gray-200">
                  ...
                  <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      {cat.icone_url ? (
                        ...
                        <img src={cat.icone_url} alt={cat.nome} className="w-10 h-10 rounded-md object-cover border border-gray-100 shadow-sm shrink-0" />
                      ) : (
                        <div className="w-10 h-10 rounded-md bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-400 shrink-0">
                          <LayoutGrid size={16} />
                        </div>
                      )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {cat.subcategorias.map((sub) => (
                        <div key={sub.id} className="flex items-start justify-between p-3 bg-white rounded-md border border-gray-200 shadow-[0_2px_10px_rgba(0,0,0,0.02)] group hover:border-[#42b883]/30 hover:shadow-md transition-all">
                          ...
                          <Button
                            variant="ghost"
                            className="h-7 w-7 p-0 rounded-md text-gray-300 opacity-0 group-hover:opacity-100 hover:text-blue-600 hover:bg-blue-50 transition-all shrink-0"
                            onClick={() => {
                ...
                {isExpanded && (!cat.subcategorias || cat.subcategorias.length === 0) && (
                  <div className="border-t border-gray-100 bg-gray-50/50 p-6 text-center">
                    <p className="text-[12px] text-gray-500 font-bold bg-white inline-block px-4 py-1.5 rounded-md border border-gray-200 shadow-sm">
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" className="rounded-md font-bold" onClick={() => { setShowCriarModal(false); criarForm.reset(); }} disabled={actionLoading}>
              Cancelar
            </Button>
            <Button variant="primary" className="rounded-md font-bold" onClick={criarForm.handleSubmit(handleCriar)} isLoading={actionLoading}>
              Criar Categoria
            </Button>
          </>
        }
      >
        <form className="space-y-5">
          <Input label="Nome da Categoria" placeholder="Ex: Construção Civil" error={criarForm.formState.errors.nome?.message} {...criarForm.register('nome')} className="rounded-md" />
          <Textarea label="Descrição" placeholder="Detalhes opcionais..." error={criarForm.formState.errors.descricao?.message} rows={3} {...criarForm.register('descricao')} className="rounded-md" />
          <Input type="file" label="Ícone da Categoria" accept="image/*" error={criarForm.formState.errors.icone?.message as string} {...criarForm.register('icone')} className="rounded-md" />
        </form>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" className="rounded-md font-bold" onClick={() => { setShowEditarModal(false); setSelectedCategoria(null); }} disabled={actionLoading}>
              Cancelar
            </Button>
            <Button variant="primary" className="rounded-md font-bold" onClick={editarForm.handleSubmit(handleEditar)} isLoading={actionLoading}>
              Guardar Alterações
            </Button>
          </>
        }
      >
        <form className="space-y-5">
          <Input label="Nome da Categoria" error={editarForm.formState.errors.nome?.message} {...editarForm.register('nome')} className="rounded-md" />
          <Textarea label="Descrição" error={editarForm.formState.errors.descricao?.message} rows={3} {...editarForm.register('descricao')} className="rounded-md" />
          <div className="space-y-2 bg-gray-50/50 p-3 rounded-md border border-gray-100">
            <Input type="file" label="Novo Ícone (Opcional)" accept="image/*" error={editarForm.formState.errors.icone?.message as string} {...editarForm.register('icone')} className="rounded-md" />
            {selectedCategoria?.icone_url && (
              <div className="mt-3 text-[12px] text-gray-500 font-semibold flex items-center gap-2">
                ...
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={selectedCategoria.icone_url} alt="Ícone atual" className="w-8 h-8 rounded-md object-cover border border-gray-200 shadow-sm" />
              </div>
            )}
          </div>
        </form>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" className="rounded-md font-bold" onClick={() => { setShowSubcategoriaModal(false); setSelectedCategoria(null); subcategoriaForm.reset(); }} disabled={actionLoading}>
              Cancelar
            </Button>
            <Button variant="primary" className="rounded-md font-bold" onClick={subcategoriaForm.handleSubmit(handleSubcategoria)} isLoading={actionLoading}>
      ...
      >
        <form className="space-y-5">
          <Input label="Nome da Subcategoria" placeholder="Ex: Reparação de canos" error={subcategoriaForm.formState.errors.nome?.message} {...subcategoriaForm.register('nome')} className="rounded-md" />
          <Textarea label="Descrição (Opcional)" placeholder="Pequeno detalhe sobre este tipo de serviço..." error={subcategoriaForm.formState.errors.descricao?.message} rows={3} {...subcategoriaForm.register('descricao')} className="rounded-md" />
        </form>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <>
            <Button variant="outline" className="rounded-md font-bold" onClick={() => { setShowEditarSubcategoriaModal(false); setSelectedSubcategoria(null); editarSubcategoriaForm.reset(); }} disabled={actionLoading}>
              Cancelar
            </Button>
            <Button variant="primary" className="rounded-md font-bold" onClick={editarSubcategoriaForm.handleSubmit(handleEditarSubcategoria)} isLoading={actionLoading}>
      ...
      >
        <form className="space-y-5">
          <Input label="Nome da Subcategoria" error={editarSubcategoriaForm.formState.errors.nome?.message} {...editarSubcategoriaForm.register('nome')} className="rounded-md" />
          <Textarea label="Descrição (Opcional)" error={editarSubcategoriaForm.formState.errors.descricao?.message} rows={3} {...editarSubcategoriaForm.register('descricao')} className="rounded-md" />
        </form>
      </Modal>
    </div>
  );
